...
Nemo is a local-first AI data exploration agent. Point it at CSV or Parquet files, give it a question (or let it explore freely), and it autonomously generates SQL, executes queries against a local DuckDB database, interprets results, builds an evidence graph of insights and contradictions, and produces a structured brief of what it found.

**Invoke when the user:**
- Has a dataset and wants to understand what's in it
- Asks an analytical question that requires multiple queries to answer
- Wants to investigate a metric change, anomaly, or pattern
- Needs a structured, auditable analysis — not a one-shot query

**Do NOT invoke for:**
- Questions answerable with a single query (just write the SQL)
- Tasks without data to analyze
- Dashboard building or visualization-only work
