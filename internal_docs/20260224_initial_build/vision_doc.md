...
Node {
  ...
  code                  // the generated Python/SQL
  result                // structured output from execution
    → type              // "table" | "chart" | "scalar" | "error"
    → data              // JSON table data, plotly spec, number, or error message
...
│    │                                              │
│    ├── Execution Sandbox                          │
│    │     → Runs generated Python/SQL              │
│    │     → Captures stdout, tables, chart specs   │
...
**Execution sandbox**: Generated Python runs in a subprocess with pandas, plotly, and the dataset pre-loaded. Charts output as Plotly JSON specs (not images) so the frontend can render them interactively. Tables output as JSON. Timeouts prevent runaway queries.

**Streaming**: The frontend subscribes to the session via SSE or WebSocket. Every time a node is created or updated, the event fires. The graph animates in real time. The feed scrolls. It feels alive.
...
2. Execute: "Given this question and dataset schema, write the code" → code
3. Integrate: "Given the result, what does this mean? What edge type connects this to existing nodes?" → answer + edges
4. Reflect (periodic): "Look at the full graph. Synthesize." → synthesis node

**Graph storage**: SQLite is fine. The graph is small (tens to low hundreds of nodes per session). NetworkX or similar is overkill for now — just nodes and edges tables with queries.
...
│   │   ├── executor.ts                // python/sql sandbox
...
class NemoEngine {
  ...
  async run(emit: (event: NemoEvent) => void) {
    ...
    while (this.status === 'diving') {
      ...
      // 2. Explore + Execute (inner ReAct loop)
      ...
      const result = await this.executor.run(code)
      const interpretation = await this.llm.interpret(result, question)

      // 3. Integrate
      const completed = { ...next, question, code, result, ...interpretation, status: 'complete' }
      ...
      if (this.shouldReflect()) {
        ...
        const synthesis = await this.reflect()
